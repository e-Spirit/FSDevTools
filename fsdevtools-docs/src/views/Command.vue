<template>
    <div class="home">
        <section id="commandDescription">
            ...
            <CommandParameters v-if="this.getParameters(true).length > 0" headline="Global parameters" :parameters="this.getParameters(true)"></CommandParameters>
            <CommandParameters v-if="this.getParameters(false).length > 0" headline="Command parameters" :parameters="this.getParameters(false)"></CommandParameters>
            <CommandExamples v-if="this.getExamples().length > 0" :examples="this.getExamples()"></CommandExamples>
        </section>
    </div>
</template>
...
import {
    ...
    getCurrentCommandName, getParameterNames
} from "@/types";
...
export default Vue.extend({
    name: 'Command',
    components: {CommandParameters, CommandHeader, CommandExamples, VueMarkdown},
    methods: {
        ...
        getParameters(global: boolean): Array<CommandParameterElement> {
            ...
            result = [...result].sort((first: CommandParameterElement, other: CommandParameterElement) => {
                ...
                return getParameterNames(first).localeCompare(getParameterNames(other));
            });
            return result;
        },
