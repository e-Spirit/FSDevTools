...
public class ServiceSerializer extends StdSerializer<Service> {
	...
	@Override
	public void serialize(@NotNull final Service value, @NotNull final JsonGenerator generator, @NotNull final SerializationContext ctxt) {
		generator.writeStartObject();
		generator.writeStringProperty(ATTR_SERVICE_NAME, value.getServiceName());
		if (value.getAutoStart()) {
			generator.writeBooleanProperty(ATTR_AUTO_START, value.getAutoStart());
		}
		if (value.getRestart()) {
			generator.writeBooleanProperty(ATTR_RESTART, value.getRestart());
		}
