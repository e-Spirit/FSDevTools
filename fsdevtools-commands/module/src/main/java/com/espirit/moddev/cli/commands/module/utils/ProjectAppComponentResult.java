...
class ProjectAppComponentResult extends ComponentResult<ProjectAppDescriptor> {
	...
	ProjectAppComponentResult(@NotNull final ProjectAppDescriptor descriptor, @Nullable final Exception exception, @NotNull final Project project) {
	...
	public String toString() {
		...
		if (exception == null) {
			...
			return String.format("Error installing/updating project app component '%s:%s' (version='%s') in project '%s': %s", getDescriptor().getModuleName(), getDescriptor().getName(), getDescriptor().getVersion(), getProject().getName(), exception.getMessage());
		}
	}
